...
export function SkillTreeNode({
  ...
  const getIcon = () => {
    if (isCompleted) return <Check className="w-7 h-7 text-white" strokeWidth={3} />;
    ...
    >
      ...
      <button
        ...
        className={cn(
          ...
          isUnlocked && "cursor-pointer transform hover:scale-125 active:scale-110",
        ...
        >
          ...
          {!isUnlocked && step.prerequisites.length > 0 && (
            <div className="text-xs text-red-600 mt-1">
              🔒 Requires {step.prerequisites.length} skill{step.prerequisites.length > 1 ? 's' : ''}
            </div>
          )}
        </div>
      </div>
