...
export function DesignsListPage() {
  ...
  const { designs, deleteDesign, updateDesign, createDesign } = useFlow();
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [designToDelete, setDesignToDelete] = useState<string | null>(null);
  const [renameDialogOpen, setRenameDialogOpen] = useState(false);
  const [designToRename, setDesignToRename] = useState<{ id: string; name: string } | null>(null);
  ...
  const handleRenameDesign = (id: string, name: string) => {
    setDesignToRename({ id, name });
    setNewName(name);
  ...
  const confirmRename = async () => {
    if (designToRename && newName.trim()) {
      try {
        await updateDesign(designToRename.id, { name: newName.trim() });
  ...
  const handleConfirmCreate = async (name: string, description: string) => {
    try {
      const newDesign = await createDesign(name, description);
      toast.success('Design created successfully');
      setCreateDialogOpen(false);
      navigate(`/designs/${newDesign.id}`);
    } catch (error) {
                  ...
                  <CardTitle className="flex items-start justify-between gap-2">
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleRenameDesign(design.id, design.name);
                        }}
                        title="Rename"
                      >
